...
namespace SimoniDoorsInventory.ViewModels
{
    ...
    public class CustomerDetailsViewModel : GenericDetailsViewModel<CustomerModel>
    {
        ...
        {
            return await DialogService.ShowAsync("Επιβεβαίωση Διαγραφής", "Σίγουρα θέλετε να διαγράψετε τον επιλεγμένο πελάτη;", "Ναι", "Ακύρωση");
        }
        ...
        protected override IEnumerable<IValidationConstraint<CustomerModel>> GetValidationConstraints(CustomerModel model)
        {
            ...
            yield return new RequiredConstraint<CustomerModel>("Phone 1", m => m.Phone1);
            yield return new RequiredConstraint<CustomerModel>("Address Line", m => m.AddressLine);
