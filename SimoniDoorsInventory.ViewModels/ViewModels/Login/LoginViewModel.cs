...
namespace SimoniDoorsInventory.ViewModels
{
    public class LoginViewModel : ViewModelBase
    {
        ...
        private Result ValidateInput()
        {
            ...
            {
                return Result.Error("Login error", "Το όνομα χρήστη δεν μπορεί να είναι κενό.");
            }
            if (string.IsNullOrWhiteSpace(Password))
            {
                return Result.Error("Login error", "Παρακαλώ, εισαγάγετε έναν έγκυρο κωδικό πρόσβασης.");
            }
            return Result.Ok();
        }
